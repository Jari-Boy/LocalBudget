 ...
 * 実効メンバーの解決(1.2節のCOALESCEルール)を経由してから絞り込み、
 * 区分をまたぐ合算はsumBalanceAcrossCategoriesに委譲し、軸(取引先・メンバー等)非依存の
...
export function aggregateByHouseholdMember(
  ...
  const lines = entries
    ...
    .filter((line) => resolveEffectiveHouseholdMemberId(line, accountsById) === householdMemberId)
