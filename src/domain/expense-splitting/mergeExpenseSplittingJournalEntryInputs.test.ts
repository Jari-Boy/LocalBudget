 ...
 * まとめて一本で仕訳を切るように(逆仕訳が切りやすくなるから)」を受け、1回の割勘確定操作
 * (分担者N人×選択した元仕訳M件)の結果を常にちょうど1件の仕訳にまとめる(計画Issue #40)。
 ...
 * 分担者ごとに個別に組み立てたCreateJournalEntryInputの配列を受け取り、明細行を1つの配列に
 * 結合し、allocatesリンクは元仕訳(to_entry_id)ごとに金額を合算して1本にまとめる
 * (docs/domain/journal.md 1.8「1回の割勘バッチが複数の元仕訳をまとめて対象にすることも
 * ある(一対多)」という既存のリンク設計をそのまま活用する)。DB非依存、外部依存なし。
 */
