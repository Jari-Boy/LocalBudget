...
**実装状況（Issue #76）**: CSV取込アップロード画面（`StatementImportUploadScreen`）・レビュー一覧画面（`StatementImportReviewScreen`、いずれも`src/components/statement-import/`）を実装した。上記方針の通りCSVパース処理（`readCsv`）はメインスレッド（React UI層）で実行し、Worker越しのRepository呼び出し（対象科目一覧・マッピング定義候補・既存突合レコードの取得）を先に完了させてから同期的にパース・マッピング変換・重複防止判定（`buildStatementImportReview`、`docs/domain/statement-import.md` 1.5〜1.6）まで行う。対象科目のマッピング定義候補は1件のみの場合に限り自動選択し、複数ある場合（楽天カードの確定/速報明細等）はユーザーが`label`で明示的に選ぶ（同1.5手順1）。この「マッピング定義選択→CSVファイル選択」という順序自体はIssue #78でCSV先選択フローに変更されている（候補の絞り込み方式・自動選択/複数選択の考え方は維持、詳細は下記「実装状況（Issue #78）」参照。本パラグラフはIssue #76時点の記述として残す）。相手勘定科目の候補は、マニュアル起票用の`isManualEntryEligibleAccount`（`is_reconcilable = true`科目を除外）とは異なる専用の`isStatementImportCounterAccountEligible`を新設して判定する。`docs/domain/reconciliation.md` 1.2の直接記帳制限ホワイトリストは`source_type = 'external_import'`を許可しているため、CSV取込由来の仕訳では口座間振替のように`is_reconcilable = true`科目同士を相手科目として選べる必要があり、マニュアル起票と同じ制約をそのまま流用すると選択できなくなる不具合になる（詳細は`docs/decisions.md`・`docs/guides/patterns.md`参照）。本Issueのスコープは「アップロード〜レビュー一覧表示」までであり、レビュー確定操作（`JournalEntryRepository`への永続化）自体は後続Issueのスコープとして未実装（画面に確定ボタンは無い）。あわせて、`docs/domain/statement-import.md` 2.3節が想定するOSS同梱の組み込みマッピング定義（楽天カード・楽天銀行・PayPayカード）をWorker起動時に投入する仕組み（`seedBuiltInMappingDefinitions`）も本Issueで先行実装した（マッピング定義作成UI自体が未実装で実機検証ができないための前倒し、詳細は`docs/decisions.md`）。

**実装状況（Issue #77）**: 上記レビュー一覧画面に、取引先推定・相手勘定科目サジェストの初期表示（`estimateCounterparty`・`suggestCounterpartyAccount`の結果をレコードの初期値として表示、`docs/domain/statement-import.md` 1.5手順4〜5、`docs/domain/counterparties.md` 1.3〜1.4）、取引先が特定できない同一摘要レコードをまとめて割り当てるインラインバナー（`groupUnresolvedRecordsByDescription`、モーダル不使用、同1.5手順6）、レビュー確定操作（「確定する」ボタン、`buildConfirmedJournalEntryInput`→`JournalEntryRepository.create`、同1.5手順7）を追加し、Issue #76時点で未実装だった永続化を完了させた。確定操作は1件が失敗しても処理を止めない継続処理方式を採り、成功/失敗件数をまとめて表示する。確定版候補（`docs/domain/statement-import.md` 1.6）は置き換え対象の旧仕訳を削除してから新規作成する。相手勘定科目に非PL科目（資産・負債）を選ぶと取引先欄が非表示・値もクリアされる整合性ガード（`resolveEligibleCounterpartyId`、`journal.sql`のDDLトリガー制約への対応）は、初期表示計算・取引先セレクト・相手科目セレクト・一括割当て適用の4経路すべてに共通適用する（一部経路のみへの実装漏れが2度連続して発生した経緯は`docs/decisions.md`・`docs/guides/patterns.md`参照）。取引先セレクトからは`CounterpartyQuickAddSelect`により新規取引先をその場で作成できる（名前のみの最小実装、詳細は`docs/decisions.md`）。プロジェクト・世帯メンバーの確認・修正UIもあわせて実装した。

**実装状況（Issue #78）**: `StatementImportUploadScreen`のCSVアップロードフローを、対象科目選択後にマッピング定義を選んでからCSVファイルを選ぶ順序から、対象科目選択後にCSVファイルを直接選ぶ順序（CSV先選択）に変更した。「取り込む」操作時に、対象科目で使える全マッピング定義候補それぞれへ実際に読み取り→変換（`resolveMappingDefinitionCandidates`という新設の純粋ドメイン関数、`src/domain/statement-import/`）を試行し、エラーなく成功した候補だけに絞り込む（`docs/domain/statement-import.md` 1.4・1.5手順1）。成功した候補が1件のみなら自動的にその定義を使い、複数あれば`label`でユーザーに選ばせ、0件なら一致するフォーマットが見つからない旨を表示する（対象科目に紐づく候補自体が0件の場合もこのエラー表示に自然に合流する）。マッピング定義選択UIは、成功候補が複数ある場合にのみ選択肢として表示される形に変わった（候補が1件以下の場合は表示しない）。evaluatorのレビュー（Attempt 1）で、複数候補から選んで確定する経路にのみ`setSubmitting(true)`の呼び出しが実装されておらず、既存突合レコード取得（`findByAccount`）の完了を待つ間ボタンが有効なままで連打による二重送信が起こりうる不具合が指摘され、Attempt 2で両経路とも分岐前に同期的に`setSubmitting(true)`するよう修正した（詳細は`docs/decisions.md`・`docs/guides/patterns.md`参照）。
