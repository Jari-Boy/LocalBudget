...
## sql.jsのdb.export()を伴う自動保存を実装すると、export()後にPRAGMA foreign_keysがリセットされCASCADE削除が機能しなくなる

**症状**: 割勘仕訳の物理削除機能を実装しPlaywright(実ブラウザ)で検証したところ、`journal_entries`を削除しても`ON DELETE CASCADE`が設定された`journal_entry_links`がカスケード削除されず残留した。Node/Vitest(`createTestDatabase`)では同一のDDL・同一シナリオで正しくカスケード削除されるため、当初は割勘固有の実装ミスを疑ったが、実際には`withAutoSave`のデバウンス保存(`db.export()`)を経由した後にのみ再現し、`db.export()`を一度も呼んでいない状態(Worker起動直後等)では発生しなかった。
**原因**: sql.jsの`db.export()`は`last_insert_rowid()`だけでなく`PRAGMA foreign_keys`もOFFにリセットする副作用を持つ(上記のパターンと同種)。`createBrowserDatabase`・`createTestDatabase`はいずれもDB接続の作成直後に`PRAGMA foreign_keys = ON`を1回設定するだけなので、`db.export()`を一度も呼ばないNode/Vitestの単体テストでは問題が起きない。一方ブラウザの実行環境では、`withAutoSave`のデバウンス保存が実行されるたびに`db.export()`が呼ばれ、以降のON DELETE CASCADE(FK制約全般)が無効化されたまま戻らない。
**対策**: `db.export()`を呼ぶ箇所(`withAutoSave`の`performSave`)で、`export()`直後に`PRAGMA foreign_keys = ON`を再設定する。この再設定には、`withAutoSave`が`scheduleSave()`をトリガーするために差し替えた後の`db.run`ではなく、差し替え前の`originalRun`を直接使う。差し替え後の`db.run`を経由すると、`PRAGMA`文自体が新たな書き込みとみなされ`scheduleSave()`が再度呼ばれ、`performSave`→`PRAGMA再設定`→`scheduleSave`→(デバウンス後)`performSave`という無限の再スケジュールに陥る。実装後は、`db.export()`を実行した後でも`ON DELETE CASCADE`が機能し続けることを直接検証する回帰テストを追加する(Node/Vitestでも`db.export()`を明示的に呼べば再現・検証できるため、ブラウザ環境専用のE2Eテストは不要)。
**該当箇所（例）**: `src/infrastructure/storage/withAutoSave.ts`(`performSave`)、`docs/guides/knowledge.md`「sql.jsのdb.export()はPRAGMA foreign_keysもOFFにリセットする副作用を持つ」、`docs/decisions.md`「sql.jsのdb.export()後はPRAGMA foreign_keysをoriginalRun経由で再設定し、ON DELETE CASCADEを保ち続ける」、計画Issue #40 Review Attempt 1(evaluator FAIL指摘)
...
- 再発例: `RecurringTransactionRuleManagementScreen`の`submitForm`・`deleteRule`・`deactivateRule`、`RecurringTransactionProposalReviewScreen`の`confirmProposal`(計画Issue #39 Implementation Attempt 1)が、同じく`Promise.resolve(fn())`のまま新規実装され、evaluatorのレビューでFAIL指摘された(4例目)。Attempt 2で4箇所とも`Promise.resolve().then(() => fn())`へ修正し(コミットffafcc9)、同期例外時に`isSubmitting`が解除されることを確認する回帰テストを追加した(コミット3599689)。新しいCRUD画面を実装するIssueが4件連続でこの罠を踏んでおり、`docs/decisions.md`に規約が明記されているだけでは実装時に参照されない状態が続いている。今後は新規CRUD画面の実装着手時(計画段階)に本パターンの箇条書きをチェックリストとして明示的に参照する運用が必要。
...
- 再発例: 計画Issue #102(勘定科目登録UIの統合フロー再設計)のE2Eテスト(`e2e/account-registration.spec.ts`)が、新規作成する科目名に`seedDefaultAccounts`の標準科目名と同じ「副業収入」を使っており、UNIQUE制約違反でテストが失敗した(コミット3f0c4db で衝突しない「臨時収入」へ変更)。`seedDefaultAccounts`自体を変更しないIssueであっても、新しいE2Eテストデータを追加する際は標準科目名との衝突を都度確認する必要があることを示す実例。
- 再発例: 計画Issue #101(開発環境限定のダミーサンプルデータ自動投入)で新設した`seedDevSampleDataIfDev`(`import.meta.env.DEV`時のみ口座3件・仕訳数件を投入)が、Playwright E2Eテストの対象である`npm run dev`の開発サーバーにも発動してしまい、「口座・仕訳0件」を前提にしたE2Eテストが11件失敗した。ただし本件のシードは本番ビルドには一切含まれない開発専用の利便性機能だったため、今回は失敗したE2Eテスト側を個別に書き換える対策は取らず、`playwright.config.ts`の`webServer.env`に`VITE_DISABLE_DEV_SEED: 'true'`を追加し、シード関数側でこの環境変数を見て投入自体を抑制する方式で解決した(詳細は`docs/decisions.md`「開発環境限定のダミーサンプルデータ自動投入(seedDevSampleDataIfDev)は...」参照)。追加する冪等シードが本番でも有効な恒久機能か開発専用の利便性機能かによって、対策(既存E2Eテスト側の修正 vs. 環境変数によるシード自体の無効化)を使い分けられることを示す実例。
- 再発例: 計画Issue #40(割勘/精算UI)の人間レビューでの追加指摘対応で新設した`seedAdvanceAccounts`(asset・liability区分にis_system_managedの「立替金」科目をそれぞれ1件投入)が、`e2e/worker-rpc.spec.ts`・`e2e/storage-adapter-boot-restore.spec.ts`の「Worker起動直後の科目一覧が標準の収益・費用科目のみと厳密一致する」というtoEqualアサーションを壊した(Review Attempt 5でevaluatorが指摘、Implementation Attempt 5コメントが報告した回帰確認はexpense-splitting関連4ファイルに限定されており、この2ファイルは対象外だった)。対策は本エントリの確立済みの方針通り、新しいシード内容(`ADVANCE_ACCOUNT_NAME`を`seedAdvanceAccounts.ts`からexportし再利用)を織り込んだ期待値へ書き換えることで解決した(Attempt 6、コミット参照)。計画Issue #96・#102に続く3件目の発生であり、evaluator自身がレビュー時に`e2e/`配下を横断的に実行して発見した(実装側が対策を実行していれば実装時点で検知できたはずの見落とし)。「実装対象のファイルだけでなくe2e配下全体を横断的にgrep・実行する」という対策を、レビュー時のセーフティネットだけに頼らず実装時に確実に実行することの重要性を示す実例。

## 複数の呼び出し元を1つに統合するリファクタで、統合後も残存する共有ヘルパー関数のdocstring・テスト説明文が旧呼び出し元の記述のまま取り残される

**症状**: `shouldShowHouseholdMemberSelectionStep`(名義選択ステップの表示要否判定)は、統合前は`AccountRegistrationWizard`(kindに応じた判定)・`CreditCardRegistrationWizard`(常にtrue)という2つの呼び出し元の違いをdocstringで説明し、「表示される場合は常に必須」という当時は両呼び出し元で共通に成り立っていた不変条件を明記していた。計画Issue #102でこの2画面を`AccountRegistrationFlow`という単一の呼び出し元に統合し、加えて「必須/任意」の切り替えを新設したにもかかわらず、関数本体・テストのdocstringは統合前の2画面を現在の呼び出し元であるかのように説明したまま、かつ新しい唯一の呼び出し元では成り立たなくなった「常に必須」という不変条件も残った状態で実装が提出された(evaluatorレビューAttempt 1でFAIL指摘、保守性上の問題)。
**原因**: リファクタの主眼(新しいフローの動作を正しく実装すること)に実装・レビュー双方の意識が向きやすく、削除されなかった既存の共有ヘルパー(呼び出し元だけが変わり、関数自体は残存する)のdocstringが「もう存在しない呼び出し元」を主語にした説明のまま残っていても、ロジック自体は正しく動くため気づかれにくい。加えて今回は、表示要否の判定ロジック自体は変更せず別の場所(呼び出し元コンポーネント側)で必須/任意の切り替えを追加したため、関数のdocstringが古い不変条件(「表示される場合は常に必須」)を主張し続けても、テストがその主張を直接検証しているわけではなく実行時には失敗しない。
**対策**: 複数の既存呼び出し元を1つの新しい呼び出し元に統合するリファクタでは、削除されるコンポーネントだけでなく、統合後も残存する共有ヘルパー関数・共通コンポーネントのdocstring・テストの説明文を、新しい呼び出し元の実態(呼び出し元の数・各呼び出し元固有の前提)に合わせて全て見直す。特に「表示される場合は常に○○」のような、当時の全呼び出し元に共通していた不変条件を明記した記述は、呼び出し元の統合・追加によってその不変条件自体が崩れていないか個別に確認する。evaluatorのレビュー時も、リファクタ対象のファイルだけでなく、リファクタ対象から呼び出される既存の共有ヘルパーのdocstringが新しい呼び出し状況と矛盾していないかを確認する。
**該当箇所（例）**: `src/components/account-registration/shouldShowHouseholdMemberSelectionStep.ts`・`.test.ts`(コミットce6ebc6で修正)、`docs/decisions.md`「HouseholdMemberSelectionStepに任意選択モード(unspecifiedOptionLabel)を追加し...」、計画Issue #102 Review Attempt 1(evaluator FAIL指摘、保守性上の問題)

## ステップ配列(steps)を状態から都度計算するウィザードで、goNextに素朴な上限クランプを実装すると新しく増えたステップに進めなくなる

**症状**: 入力(entryChoice/hasExternalStatement等)によってステップ数・内容が動的に変わるステップベースのウィザード(`AccountRegistrationFlow`)で、あるステップのボタンのonClickハンドラが「状態の更新(`setEntryChoice`等)」と「次のステップへ進む(`goNext`)」を同一ハンドラ内で同時に呼ぶ設計にすると、`goNext`が呼ばれた時点でそのレンダーが計算した`steps`配列はまだ直前の`setState`が反映される前の(更新前の)状態を基にしたものになっている。この状態で`goNext`に`Math.min(index + 1, steps.length - 1)`のような素朴な上限クランプを実装すると、更新後は本来ステップ数が増えるはずの箇所で更新前の(短い)`steps.length`を基準に頭打ちになり、以後ステップが一切進まなくなる。
**原因**: Reactの1回のレンダー内では、直前の`setState`呼び出しがまだ反映されておらず、そのレンダーで計算された派生値(本件の`steps`)は「更新前の状態」を基にしたものである。`goNext`の呼び出し元(onClickハンドラ)は状態更新と同一ハンドラ内にあるため直感的には「更新後のstepsを基準にクランプすればよい」ように見えるが、実際にはハンドラ実行中のクロージャが参照する`steps`は常に更新前の値であり、次のレンダーで初めて更新後の`steps`が計算される。
**対策**: ステップ配列が状態によって動的に変わる(入力ごとに配列の長さ・内容が変わる)ウィザードでは、`goNext`側で`steps.length`を基準にした上限クランプを実装せず単純なインクリメントに留める。次のレンダーで新しい状態から再計算された`steps`に対して`stepIndex`が参照されるため、クランプを省略しても表示側(`steps[stepIndex]`)は安全に動作する。範囲外アクセス防止が必要な場合は`goNext`側ではなく表示側(`steps[stepIndex] ?? 最終ステップ`等)で行う。
**該当箇所（例）**: `src/components/account-registration/AccountRegistrationFlow.tsx`(`goNext`実装直前のコメント)、`docs/decisions.md`「ステップベースのウィザードでsteps配列を状態から都度計算する場合、goNextに上限クランプをかけない」、計画Issue #102実装中に自己発見(evaluator指摘ではない)

## 冪等シード関数がis_system_managed等のフラグの有無だけで判定すると、既存の非システム管理レコードとの名前衝突(UNIQUE制約)でWorker起動全体がクラッシュしうる

**症状**: `seedAdvanceAccounts`(Worker起動時にasset/liability区分へ`is_system_managed = true`の「立替金」科目を自動投入する冪等シード)は、投入要否の判定を「その区分の`is_system_managed`科目が1件も無いか」のみで行っていた。ユーザーが過去に(または別の経路で)同じ区分に同名(「立替金」)の通常科目(`is_system_managed = false`)を既に作成済みだった場合、投入処理の`INSERT`が`category`+`name`のUNIQUE制約(`docs/schema/accounts.sql`)に違反し例外を投げる。この例外を`db.worker.ts`の`main()`内で捕捉していなかったため、Worker起動処理全体が失敗し、割勘機能どころかアプリ全体が起動できなくなっていた(evaluatorレビューAttempt 5で指摘・実機相当のテストで再現)。
**原因**: 冪等シードの「投入済みかどうか」の判定条件(本件は`is_system_managed`フラグの有無)は、シードが投入するレコード自体の状態のみを見ており、同じ一意制約(`category`+`name`)を共有しうる「シード対象ではない既存レコード」との衝突までは考慮していなかった。`seedDefaultAccounts`(収益・費用科目)等の既存の冪等シードは「その種類のレコードが1件でもあれば投入しない」という粗い判定のため同名衝突が原理的に起こりにくかったのに対し、本件は「is_system_managedのものだけを見る」という細かい判定にしたことで、is_system_managed = falseの同名レコードとの共存を許してしまっていた。
**対策**: ユーザーが自由にレコードを作成できる区分・マスタに対して、特定のフラグを持つレコードのみを対象にした冪等シードを追加する場合、フラグを持たない既存レコードとの名前衝突(UNIQUE制約違反)が起こりうることを踏まえ、投入処理(`INSERT`)を個別に`try/catch`で囲み、失敗時はログに残すだけでWorker起動処理自体は継続させる(その区分・種類のシードだけを諦め、他のシード・アプリ起動全体には影響させない)。呼び出し元(`db.worker.ts`の`main()`)側で例外を吸収しようとするのではなく、シード関数自身の内部で衝突を許容する設計にすることで、影響範囲をシード関数自身に局所化できる。実装時は、同名の非システム管理レコードが事前に存在する状態でシード関数を呼び出しても例外を投げず(`expect(() => seed(db)).not.toThrow()`)、既存レコードが書き換えられないことを検証する回帰テストを追加する。
**該当箇所（例）**: `src/infrastructure/db/seedAdvanceAccounts.ts`(`seedIfMissing`内の`try/catch`)、`src/infrastructure/db/seedAdvanceAccounts.test.ts`(「既に同名(is_system_managed = false)の資産科目が存在する場合...」)、`docs/decisions.md`「seedAdvanceAccountsは科目名衝突(UNIQUE制約違反)をログに残すだけでスキップし、Worker起動全体をクラッシュさせない」、計画Issue #40 Review Attempt 5(evaluator指摘)

## 除外条件が行の種類によって異なる複数種類の入力行を許容フィルタで確定リストへ変換すると、特定の種類だけが無警告に全滅しても集約後の空チェックでは検出できない

**症状**: 割勘起票フォーム(`ExpenseSplittingForm`)の分担者行は世帯メンバー・世帯外相手の混在を許すが、確定リストへの変換関数(`toExpenseSplitRecipients`/`toExpenseSplitRecipientsForEntryAmount`)は「世帯メンバー行は`advanceLiabilityAccountId`(立替金負債科目)が無いと仕訳を組み立てられない」という理由で、対象がnullの場合その行を警告なく除外(スキップ)していた。フォームの送信時バリデーションは「変換後のrecipients配列が0件かどうか」(`noRecipientError`)しか見ていなかったため、世帯外相手の行が1件でも有効であれば、世帯メンバー行だけが無警告で消失していてもエラーにならず、ユーザーが意図した割勘の一部(世帯メンバー分)が欠落したまま確定されてしまう。evaluatorレビュー(Review Attempt 1)で指摘された。
**原因**: 「不完全な入力行は確定対象から除外する」という寛容なフィルタリング方針自体は、単一種類の行(除外条件が全行で同一)を前提にすれば「フィルタ後に0件になっていないか」という集約チェックだけで安全に機能する。しかし本件のように除外条件が行の種類によって異なる(世帯メンバー行のみ`advanceLiabilityAccountId`の有無に依存し、世帯外相手の行は依存しない)場合、集約後の「0件かどうか」という粗いチェックでは、特定の種類の行だけがまるごと消えていても、他の種類の行が1件でも生き残っていれば検出できない。
**対策**: 複数種類の行を許容するフィルタで、種類ごとに異なる前提条件(欠落しうる外部依存)がある場合、確定後のリスト全体の空チェックだけに頼らず、各種類について「その種類の行が1件でも入力されているのに、前提条件が満たされず全て脱落する」ケースを個別に検出し、専用のエラーメッセージで警告する。実装計画時点で「このフィルタは行の種類によって除外条件が異なるか」を確認し、異なる場合は種類ごとの事前検証を確定処理の入り口に追加する。テストは「特定の種類の行だけが前提条件を満たさず、かつ他の種類の行は有効」という組み合わせを明示的に用意し、警告が表示されることを検証する。
**該当箇所（例）**: `src/components/expense-splitting/ExpenseSplittingForm.tsx`(`handleSubmit`内の`hasHouseholdMemberParticipant`事前検証)、計画Issue #40 Review Attempt 1(evaluator指摘、コミットdc2e071で修正)
- 再発例: 同じ`ExpenseSplittingForm`で、今度は世帯外の相手(`kind === 'counterparty'`)の行が`targetId === null`(取引先未選択)のまま確定できる不具合が、evaluatorのPASS後の人間による実機レビューで発見された。原因・対策とも上記と全く同型(除外条件が行の種類によって異なり、`noRecipientError`の集約チェックだけでは特定の種類の全滅を検出できない)で、`hasIncompleteCounterpartyParticipant`という同じ形の専用事前検証(`counterpartyTargetRequiredError`)を追加して対応した(コミット8ec73f2、詳細は`docs/decisions.md`(2026-09-06)参照)。evaluatorのレビューをPASSしていても、同一コンポーネント内に同型の欠陥が行の種類ごとに複数個所独立して仕込まれうることを示す実例であり、1箇所修正して終わりにせず、同じ関数・同じコンポーネント内の他の行の種類にも同種の事前検証が漏れなく揃っているか横断的に確認する価値がある。

## findAll()等の配列の並び順(id採番順=挿入順)を、業務上意味のある順序(取引日順等)であるかのように暗黙に前提して先頭要素を代表値に使う

**症状**: `listExpenseSplittingHistory`(複数の元仕訳をまとめて1回で割勘した履歴一覧の構築)が返す`originalEntries`配列について、UI側(`ExpenseSplittingHistoryScreen`)が`originalEntries[0].entryDate`を「元の支出の日付」として表示していた。`originalEntries`は`SqlJsJournalEntryRepository.findAll()`(`ORDER BY id`)が返す仕訳をentries引数の登場順のまま`push`していただけで、`entryDate`による並び替えは行っていなかった。通常はCSV取込順と取引日順が一致するため一見正しく動くが、銀行明細CSVが新しい日付から降順で並ぶ場合等、取込順(id昇順)と取引日順が逆転する組み合わせをまとめて割勘すると、最古でも最新でもない「単にDBへ先に挿入されただけ」の日付が表示される。evaluatorのレビュー(Review Attempt 2)では、既存テストが元仕訳の作成順序と日付の昇順がたまたま一致するfixtureだったため、この設計漏れが検出されないまま一度症状のみの修正(日付の参照元をsplitEntryからoriginalEntries[0]へ変えるだけ)で通過しかけたことも判明した。
**原因**: `id`のような技術的な連番(挿入順)と、ドメイン上意味のある順序(取引日等)は多くの操作パターンでたまたま一致するため、明示的にソートしなくても「並んでいるように見える」。この暗黙の一致に頼った実装は、一致が崩れる入力(過去日付データの後発インポート等)が実際に発生するまでテストでも顕在化しない。
**対策**: Repositoryの`findAll()`等が返す配列、またはそれをグルーピングしただけの配列から「先頭要素」「代表要素」を選んで表示・計算に使う場合、その配列の並び順が技術的な連番(id採番順=挿入順)なのか業務上意味のある順序(日付順等)なのかを必ず確認する。後者が必要な場合は、呼び出し側やUI層の判断に委ねず、その配列を返すドメイン層の関数自身が対象フィールドで明示的にソートしてから返す。テストは、配列内の登場順と期待する業務上の順序が意図的に逆転するfixture(本件は取込順で新しい日付を先に、古い日付を後に配置したケース)を用意し、順序への暗黙の依存を検出できるようにする。
**該当箇所（例）**: `src/domain/expense-splitting/listExpenseSplittingHistory.ts`(`entryDate`の`localeCompare`昇順ソート、コミットb69d153)、`src/domain/expense-splitting/listExpenseSplittingHistory.test.ts`(登場順と取引日順を意図的に逆転させたfixture)、`docs/decisions.md`「listExpenseSplittingHistoryのoriginalEntriesはentryDate昇順にソートして返し、findAll()の返り順(id順)に依存しない」、計画Issue #110 Review Attempt 2(evaluator FAIL指摘、重大度MEDIUM)・Attempt 3で修正。`docs/guides/patterns.md`「複数フィールドが共通の候補プールから割り当てを行う推測ロジックで、1パスの処理順ベース実装にすると結果が処理順に依存する」とは処理順・配列順への暗黙の依存という点で同種だが、本件は「複数フィールドの排他的割当て」ではなく「配列から代表要素を選ぶ際の並び順の意味の取り違え」である点が異なる。

## 親子関係を持つ集約で、同一フォーム内の「親を選ぶ欄」と「子を選ぶ欄」を独立に実装すると循環参照になる組み合わせが選択可能になる

**症状**: 勘定科目グループの新規作成フォーム(`AccountGroupManagementScreen`)に、新規グループの親を選ぶ`<select>`と、既存グループを新規グループの子として一括指定するチェックボックス一覧を追加した際、両者を独立したUI状態(`parentGroupIdInput`・`childGroupIdsInput`)として実装した。この実装では、新規グループの親としてグループBを選びつつ、Bの祖先であるグループAを子として同時に選択する、という循環参照になる組み合わせがUI上は制限なく選べてしまう。送信するとグループ自体の作成は成功するが、続くAの親付け替え(`update(A, { parentGroupId: 新規グループ.id })`)だけがRepository層の循環参照検証(`isDescendantGroup`)に拒否され、新規グループは作成済みなのに子への付け替えは失敗するという中途半端な状態になる。evaluatorのレビューで指摘された。
**原因**: 「親を選ぶ欄」と「子を選ぶ欄」はUI上は見た目も操作も独立した別々のコントロールに見えるため、実装時に両者の組み合わせが循環参照を作りうることに気づきにくい。Repository層(インフラ層)には既に確立済みの`isDescendantGroup`による循環参照防止があるため、「最終的にはRepository層が守ってくれる」という安心感から、UI側での事前排除が漏れやすい。
**対策**: 親子関係(木構造)を持つ集約で、同一フォーム内に親を選ぶ欄と子を選ぶ欄を両方設ける場合、両者を無関係な独立したUI状態として実装せず、一方の選択値(親グループ)に応じてもう一方(子グループ候補)を動的に絞り込む(選択中の親の祖先・親自身を候補から除外する)。加えて、フォームの状態(チェック済みの子)が候補の変化に追従しない可能性があるため、送信時にも最新の候補集合との積集合を取ってから送信する。実装計画時点で「このフォームに含まれる複数の選択欄は、木構造上の親子関係のように互いに排他的な制約を持つか」を確認し、持つ場合は候補の相互絞り込みをテストで検証する(親を選んだ状態で、その祖先が子候補に含まれないことを確認するテストケースを用意する)。
**該当箇所（例）**: `src/components/account-group-management/AccountGroupManagementScreen.tsx`(`childGroupCandidates`が`parentGroupIdInput`の祖先を`isDescendantGroup`で除外)、`docs/domain/accounts.md` 5.3、`docs/decisions.md`「新規グループ作成フォームで親グループ選択と子グループ一括指定を同一フォームで独立に扱わず…」、計画Issue #112(evaluator FAIL指摘、コミットa205f23で修正)

## ルーティング導入後、遷移前後の2画面が同じラベル文字列を持つフィールドを別々に持つと、E2Eテストの非strictなロケータが遷移中に前画面側の要素を誤って掴む

**症状**: 割勘対象選択画面(`ExpenseSplittingEntryPickerScreen`、内部で`JournalEntryFilterForm`の絞り込み欄を使う)から割勘起票フォーム(`ExpenseSplittingForm`)へ`navigate()`で遷移するE2Eテストで、`page.getByLabel('プロジェクト').selectOption(...)`が正しく選択されず、送信時に「プロジェクトを選択してください。」のバリデーションエラーになりテストがflakyに失敗した。`JournalEntryFilterForm`の絞り込み用`<select>`(`id="journal-entry-filter-project"`)と、`ExpenseSplittingForm`内の`ProjectQuickAddSelect`(実際に選択したい対象)は、どちらもラベル文字列が「プロジェクト」で一致しており、Playwrightの`getByLabel`はデフォルトで非strict(DOM順で最初に見つかった要素に解決する)。react-router導入前(`Screen`型union + `useState`による条件分岐)では画面の切り替えが同一コミット内の同期的なReact再レンダーで完結し、旧画面と新画面のDOMが同時に存在する瞬間が実質的に無かったため顕在化しなかったが、react-router導入後は`navigate()`による遷移の際に旧画面(遷移元)のDOMがまだ完全に取り除かれていない一瞬が生じることがあり、Playwrightがその一瞬に非strictなロケータで要素解決すると、遷移先ではなく遷移元の同名ラベル要素を掴んでしまう。実機のPlaywright trace(`show-trace`)で、`getByLabel('プロジェクト')`が`journal-entry-filter-project`というidの要素に解決されていたことが直接の証拠として確認できた。
**原因**: 複数の画面が同じ概念(「プロジェクト」等)を指す同名ラベルのフィールドをそれぞれ独立に持つこと自体はUI設計上ごく自然だが、E2Eテストのロケータが画面をまたいだ一意性を保証していない(`page.getByLabel(...)`はページ全体から探す)。ルーティングライブラリ導入によって画面遷移が「同期的な状態切り替え」から「非同期を伴いうるルート遷移」に変わったことで、両画面のDOMが一瞬共存しうるタイミングウィンドウが新たに生まれ、この暗黙の前提(「同じラベルの要素は常に1画面に1つしか無い」)が崩れた。
**対策**: E2Eテストで特定の画面の要素を操作する際は、ページ全体を対象にした`page.getByLabel(...)`/`page.getByRole(...)`ではなく、その画面自身を一意に特定できるコンテナ(ルート要素のclassName等)にスコープしたロケータ(`page.locator('.expense-splitting-form').getByLabel('プロジェクト')`等)を使う。特に、遷移元・遷移先のいずれかがフィルタ・絞り込みフォームのような「他画面でも使われがちな共通ラベル」を持つ場合は要注意。実装計画時点で「これから操作する画面と、直前の画面(遷移元)は同じラベル文字列のフィールドを持つか」を確認し、持つ場合はコンテナスコープを付ける。ルーティング導入・変更に伴うE2E修正では、単に遷移ホップ数を合わせるだけでなく、遷移前後の画面が共有しうるラベル・見出しの衝突が無いかを横断的に確認する価値がある。
**該当箇所（例）**: `e2e/expense-splitting.spec.ts`(`openSplittingForm`等を`.expense-splitting-form`にスコープ)、`src/components/journal-entry/JournalEntryFilterForm.tsx`(`id="journal-entry-filter-project"`)、`src/components/expense-splitting/ExpenseSplittingForm.tsx`(`ProjectQuickAddSelect`)、`docs/decisions.md`「react-routerを導入しナビゲーション基盤をルーティングベースに刷新し、HashRouterを採用する」、計画Issue #118

## 「最新の実績」だけからチェックポイントを逆算する設計は、確認・処理の順序が昇順でないと未処理項目が実績に上書きされて消失する

**症状**: 定期取引の提案評価(`evaluateRecurringTransactionProposals`)は、「前回チェック日」を専用カラムとして持たず、ルールごとに「生成済みの`journal_entries`のうち最新の`entry_date`」から都度逆算する設計にした(`max_occurrences`の残数をCOUNTから逆算する既存方針と同じ「二重管理を避ける」考え方)。この設計自体は妥当だが、提案の確認(`confirm`、対象日を選んで仕訳を生成する操作)が対象日の昇順を守らない実装のままだと、ユーザーが複数の保留中対象日のうち最新の1件だけを先に確認した場合、逆算の起点である「最新の`entry_date`」がその対象日まで一気に進んでしまい、飛び越された古い未確認の対象日が二度と保留中として評価されなくなる(実績が「最新の1件」だけで進捗を表現するため、どの対象日が未確認のまま残っているかを区別できない)。evaluatorのレビュー(計画Issue #121 Review Attempt 1、FAIL指摘)で実際に再現された。
**原因**: 「実績(生成済みレコード)からチェックポイントを逆算する」設計は、確認・処理が常に時系列の昇順で行われることを暗黙の前提にしている。この前提はドメインドキュメントにも実装コメントにも明示されないまま、逆算ロジック自体(`evaluateRecurringTransactionProposals`)の正しさにのみ意識が向きやすく、それを呼び出す側の操作(`confirm`)がその前提を破りうる(任意の対象日を選んで確認できてしまう)ことは見落とされやすい。「最新の実績から逆算する」という設計判断と、「その実績を更新する操作の順序を制約する」という設計判断は本来セットで検討すべきだが、前者だけが検討され後者が抜け落ちた。
**対策**: 「最新の実績(最大値・最新日付等)からチェックポイントを逆算する」設計を採用する場合は、その実績を新規に積み増す操作(本件の`confirm`)が必ず昇順(最も古い未処理のものから順)にしか実行できないことを、呼び出し側のバリデーションとして明示的に強制する。具体的には、操作の入力(本件の`dueDate`)を受け取った直後に、現在保留中の項目一覧を再評価し「最も古い保留中の項目」と一致するかを確認してから初めて実績への書き込みを許可する。実装計画時点で「このチェックポイントは実績からの逆算方式か」「その実績を更新する操作は複数の未処理項目からどれか任意の1件を選べる設計になっていないか」を確認し、任意選択を許す設計であれば昇順制約の要否を検討する。テストは、複数の保留中項目がある状態で最新(または中間)の項目だけを先に確認しようとした場合にエラーになることと、その後も古い項目が保留中のまま残り続けることの両方を検証する。
**該当箇所（例）**: `src/infrastructure/rpc/createRecurringTransactionProposalApi.ts`(`confirm`内の「最も古い保留中の対象日」検証、コミット56b4fcf)、`src/domain/recurring-transaction/evaluateRecurringTransactionProposals.ts`、`docs/domain/recurring-transactions.md` 1.2「『前回チェック日』はルールごとに専用の値を永続化せず、生成済みの実績から逆算する」、`docs/decisions.md`「定期取引の『前回チェック日』は専用の永続化フィールドを持たず生成済み仕訳の実績から逆算し、提案の確認(confirm)は保留中の対象日の昇順のみ許可する」、計画Issue #121 Review Attempt 1(evaluator FAIL指摘)
