...
本ファイルは全体方針をまとめた**エントリポイント**であり、各ドメインの詳細(体系・ライフサイクル・マスタのフィールド定義・制約の設計判断)は[domain/](./domain/)配下のファイルに分割している([2章](#2-各ドメインの詳細)参照)。実際のCREATE TABLE等のDDLは[schema/](./schema/)配下に分離しており、各ドメインファイルからリンクで参照する。
...
| 取引先(Counterparty) | 「どこで/誰に対しての取引か」を表すタグ、`counterparties`の1レコード。外部明細取込時の相手勘定科目のデフォルト推定にも用いる([counterparties.md 1章](./domain/counterparties.md#1-取引先)参照) |
...
| 取引先 | 「どこで/誰に対しての取引か」を表す軸。外部明細取込の自動推定・勘定科目のデフォルトサジェストに利用 | [domain/counterparties.md](./domain/counterparties.md) |
...
| 外部明細取込 | 金融機関の明細(CSV等)の中間表現への正規化、データ駆動のマッピング定義(`import_mapping_definitions`)、パースからレビュー確定までのフロー、Reader/マッピング定義/レビュー画面/Repositoryの責務分担。**たたき台** | [domain/statement-import.md](./domain/statement-import.md) |
