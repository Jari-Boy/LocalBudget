...
> [reconciliation.md 1.2](./reconciliation.md#12-is_reconcilable資産負債への直接記帳の制限)の通り、`is_reconcilable = true`の科目は外部明細取込由来の仕訳からしか使えない。「未収金」は給与等をCSV到着前に暫定計上するための一時勘定であり、CSVと直接照合される本物の口座ではないため、口座登録ウィザード([4章](#4-口座登録のux))ではなく通常の勘定科目追加で`is_reconcilable = false`として作成する([settlement.md](./settlement.md)参照)。`is_reconcilable = true`にしてしまうと未収金自体への暫定計上ができなくなり本末転倒になる。
>
> **「クレジットカード未払金」もis_reconcilable = falseで登録する**
> クレジットカードの利用明細はCSVという外部の正が存在する点で銀行口座と似ているが、支払い(消込)の経路は口座振替に限らず、コンビニでの現金払いのように外部明細を伴わない経路もあるため、記帳経路を外部明細取込だけに限定できない([5.3 なぜis_reconcilable = falseにするか](#53-なぜis_reconcilable--falseにするか)参照)。したがって専用のウィザード([5章 クレジットカード登録のUX](#5-クレジットカード登録のux))で`is_reconcilable = false`の負債科目として登録する。ただし家賃等の暫定計上に使う汎用の一時勘定([3.1](#31-フィールド定義)参照)とは異なり、カードごとに専用の科目を持たせる点は変わらない([5.2 なぜカードごとに専用科目にするか](#52-なぜカードごとに専用科目にするか)参照)。
...
| 科目統合(複数科目を1科目にまとめる) | 統合先科目を選ぶ | 対象科目に紐づく`journal_lines.account_id`を統合先へ一括UPDATE。[2.2](#22-過去仕訳の付け替え)参照 |
| 非アクティブ化 | 任意 | 過去集計はそのまま表示、新規仕訳では選択不可 |

### 2.2 過去仕訳の付け替え

「バス代」「電車賃」を「交通費」にまとめたい、といった科目統合(複数科目を1科目に集約する操作)は、対象科目に紐づく`journal_lines.account_id`を統合先科目へ**直接UPDATE**することで行う。FSは仕訳から都度生成されるため、付け替えても次の集計で自動反映され、FS側の特別対応は不要。

> **なぜ反対仕訳を使わないか**
> 当初は[journal.md 1.8](./journal.md#18-仕訳間の関係journal_entry_links)の`reverses`(反対仕訳)で「元仕訳を打ち消し、新科目で再計上する」方式を検討したが、これは統合対象の仕訳1件につき「元仕訳+反対仕訳+再計上仕訳」の3件に膨れ、仕訳一覧のノイズが増える。バス代/電車賃の科目自体を切り替えるという行為は、取引の経済的な事実(いつ・いくら・何に使ったか)を何も変えないラベルの貼り替えに過ぎないため、[1.5 仕訳の編集・削除](./journal.md#15-仕訳の編集削除)の「日常的な訂正は直接編集で完結させる」という方針と同じ考え方で、直接UPDATEを採用する。この判断により`journal_entry_links.reverses`はどのユースケースからも使われなくなったため、設計そのものから削除した([journal.md 1.8](./journal.md#18-仕訳間の関係journal_entry_links)参照)。
...
このログは`journal_entries`側には一切影響しない(仕訳自体は増減しない)独立したテーブルであり、[1.8](./journal.md#18-仕訳間の関係journal_entry_links)の「フィールド単位の変更履歴は持たない」という仕訳側の方針とは別に、科目マスタの再編という低頻度の操作に限定した軽量な履歴として位置づける。
...
| `is_reconcilable` | 照合可否 | 資産・負債科目は`NOT NULL`、他区分は`NULL`必須(CHECK制約で強制)。「自身の外部明細だけで残高が完結するか」が基準であり([reconciliation.md 1.1](./reconciliation.md#11-位置づけ)参照)、現状は銀行口座等の資産科目のみ`true`になりうる。クレジットカード未払金([5章 クレジットカード登録のUX](#5-クレジットカード登録のux)参照)は`liability`区分だが、消込の経路が口座振替に限らないため`false` |
...
「初期残高10万円」の入力で、まず口座専用の初期残高科目(`category = 'equity'`, `is_system_managed = true`, `initial_balance_for_account_id` = 対象の資産科目ID、[3.1](#31-フィールド定義)参照)が自動生成され、続けて以下の仕訳が裏で生成される。ユーザーは純資産という概念を理解する必要がない。この仕訳の`source_type`は`initial_balance`とする([journal.md 1.7 作成経路(source_type)](./journal.md#17-作成経路source_type)、[reconciliation.md 1.2](./reconciliation.md#12-is_reconcilable資産負債への直接記帳の制限)参照)。
...
|---|---|
| 「クレジットカードを登録する」ボタン | ― |
| ① 名前を付ける(例:楽天カード) | `category = 'liability'`, `is_reconcilable = false`が固定・非表示ですべて。`name = "楽天カード"` |
| ② 名義を選ぶ(任意、例:夫/妻/夫婦/共通) | `household_member_id`を設定([4.1](#41-ウィザードと裏側の対応)の口座登録と同じ継承ルール) |

口座登録の「① 種類を選ぶ」([4.1](#41-ウィザードと裏側の対応)参照)に相当するステップは無い(クレジットカードという種類自体が`category = 'liability'`かつ専用ウィザード経由という組み合わせに一意に対応するため)。「④ 初期残高を入力」に相当するステップもMVPでは設けない。銀行口座と異なり、未精算額は外部明細取込を重ねるうちに自然に積み上がるため、初期値を入力する必要性が薄い。

### 5.2 なぜカードごとに専用科目にするか

全カードで単一の「未払金」科目を共有すると、[statement-import.md 1.6 重複防止フロー](./statement-import.md#16-重複防止フロー)の`external_id`重複判定がカード横断になり、どのカードの明細かを区別できなくなる。[3.1](#31-フィールド定義)で口座ごとに初期残高科目を分けた判断([なぜ口座ごとに初期残高科目を分けるか](#31-フィールド定義)参照)と同じ発想で、カードごとに専用の未払金科目を作る。突合(重複防止)は`is_reconcilable`の値に関係なく、外部明細を取り込んだ科目に対して常に働く独立した仕組みであるため([reconciliation.md 1.1](./reconciliation.md#11-位置づけ)参照)、この効果は`is_reconcilable`の値によらず得られる。
