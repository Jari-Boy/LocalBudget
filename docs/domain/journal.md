...
> **「開始した時点」とは、フォームの表示(マウント)ではなくユーザーが最初に入力した時点を指す**
> `JournalEntryForm`(Issue #32、マニュアル起票UI)の実装では、フォームを表示しただけでは下書きを作成せず、日付・摘要・明細行のいずれかを最初に変更した時点で(デバウンス自動保存経由で)初めて`journal_entry_drafts`へ作成する。フォームを開いただけで何も入力せず離脱する操作のたびに空の下書きが蓄積することを避けるための実装判断であり、本節冒頭の「開始した時点」はこの意味で解釈する。詳細は`docs/decisions.md`参照。
